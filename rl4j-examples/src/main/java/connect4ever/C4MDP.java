...
public class C4MDP implements MDP<C4MDP.GameObservation, Integer, DiscreteSpace>
{
    ...
    public C4MDP() // the MDP referenced by the NN
    {
        ...
        int[] shape = {6, 7, 1}; //6x7 array with 1D of depth
    ...
    {
       c4board.reset();
       return new GameObservation(matrixBoard);
    }
    ...
    public StepReply<GameObservation> step(Integer action)
    {
        ...
        if (c4board.isGameOver() == false)
        {
            c4board.addPiece(opponent.move(c4board.getBoardState()), 2);
            if (c4board.isGameOver()==true)
            {
        ...
        return new StepReply(new GameObservation(matrixBoard), r, c4board.isGameOver(), null);
    }
    ...
    public static class GameObservation implements Encodable
    {
        ...
        public GameObservation(int[][] matrixBoard)
        {
            flatBoard = new double[42];
